...
// Migration script to add isRenewed field to all existing tax records
async function addIsRenewedFieldToTax() {
  try {
    // Connect to MongoDB
    ...
    const recordsWithoutField = await Tax.countDocuments({
      isRenewed: { $exists: false }
    })

    console.log(`📊 Found ${recordsWithoutField} tax records without isRenewed field`)

    if (recordsWithoutField === 0) {
      console.log('✅ All tax records already have isRenewed field!')
      process.exit(0)
    }
    ...
    const recordsWithField = await Tax.countDocuments({
    ...
    if (totalRecords === recordsWithField) {
      console.log('\n🎉 All tax records now have isRenewed field!')
    }
