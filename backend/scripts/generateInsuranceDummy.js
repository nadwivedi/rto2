...
// Generate 30 dummy insurance records
const generateInsuranceDummy = async () => {
  try {
    ...
    for (let i = 1; i <= 30; i++) {
      ...
      const totalFee = Math.floor(Math.random() * (15000 - 5000 + 1)) + 5000 // 5000-15000
      ...
      insuranceRecords.push({
        ...
        remarks: Math.random() > 0.7 ? 'Comprehensive coverage' : ''
      })
    }

    // Delete existing records (optional - remove this if you want to keep existing data)
    await Insurance.deleteMany({})
    console.log('🗑️  Cleared existing insurance records')

    // Insert new records
    const result = await Insurance.insertMany(insuranceRecords)
    console.log(`✅ Successfully inserted ${result.length} insurance records`)
    ...
    console.error('❌ Error generating insurance dummy data:', error)
  } finally {
