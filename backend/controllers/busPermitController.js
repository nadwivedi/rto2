...
const getBusPermitStatus = (validTo) => {
...
exports.createPermit = async (req, res) => {
  try {
    ...
    // Validate bus-specific fields
    if (!routeFrom || !routeFrom.trim()) {
      return res.status(400).json({
        success: false,
        message: 'Route from is required',
    ...
    if (!routeTo || !routeTo.trim()) {
      return res.status(400).json({
        success: false,
        message: 'Route to is required',
    ...
    // Create new Bus permit
    const newPermit = new BusPermit(permitData)
    await newPermit.save()

    res.status(201).json({
      success: true,
      message: 'Bus permit created successfully',
      data: newPermit
    })
  } catch (error) {
    console.error('Error creating Bus permit:', error)
...
exports.getAllPermits = async (req, res) => {
  try {
    ...
    // Execute query
    const permits = await BusPermit.find(query)
      .sort(sortOptions)
      .skip(skip)
    ...
    const total = await BusPermit.countDocuments(query)
    ...
    console.error('Error fetching Bus permits:', error)
    logError(error, req)
    const userError = getUserFriendlyError(error)
    res.status(500).json({
      success: false,
...
// Export all Bus permits without pagination
exports.exportAllPermits = async (req, res) => {
  try {
    const permits = await BusPermit.find({ userId: req.user.id })
      .sort({ createdAt: -1 })
    ...
    console.error('Error exporting Bus permits:', error)
    res.status(500).json({
      success: false,
      message: 'Failed to export Bus permits'
    })
  }
}

// Get expiring soon Bus permits
exports.getExpiringSoonPermits = async (req, res) => {
  try {
    ...
    const vehiclesWithActivePermits = await BusPermit.find({ status: 'active', userId: req.user.id })
    ...
    const permits = await BusPermit.find(query).sort(sortOptions).skip(skip).limit(parseInt(limit)).lean();
    ...
    console.error('Error fetching expiring soon Bus permits:', error);
    res.status(500).json({ success: false, message: 'Error fetching expiring soon Bus permits' });
  }
};

// Get expired Bus permits
exports.getExpiredPermits = async (req, res) => {
  try {
    ...
    console.error('Error fetching expired Bus permits:', error);
    res.status(500).json({ success: false, message: 'Error fetching expired Bus permits' });
  }
};

// Get pending payment Bus permits
exports.getPendingPermits = async (req, res) => {
  try {
    ...
    console.error('Error fetching pending Bus permits:', error);
    res.status(500).json({ success: false, message: 'Error fetching pending Bus permits' });
  }
};
...
exports.getPermitById = async (req, res) => {
  try {
    ...
    const permit = await BusPermit.findOne({ _id: id, userId: req.user.id })

    if (!permit) {
      return res.status(404).json({
        success: false,
        message: 'Bus permit not found'
      })
    }
    ...
    console.error('Error fetching Bus permit:', error)
...
exports.getPermitByNumber = async (req, res) => {
  try {
    ...
    const permit = await BusPermit.findOne({ permitNumber, userId: req.user.id })
...
exports.updatePermit = async (req, res) => {
  try {
    ...
    // Validate bus-specific fields if provided
    if (routeFrom !== undefined && (!routeFrom || !routeFrom.trim())) {
      return res.status(400).json({
        success: false,
        message: 'Route from cannot be empty',
        errors: ['Route from cannot be empty'],
        errorCount: 1,
        timestamp: getSimplifiedTimestamp()
      })
    }

    if (routeTo !== undefined && (!routeTo || !routeTo.trim())) {
      return res.status(400).json({
        success: false,
        message: 'Route to cannot be empty',
        errors: ['Route to cannot be empty'],
        errorCount: 1,
        timestamp: getSimplifiedTimestamp()
      })
    }
    ...
    if (validTo !== undefined) {
        ...
        updateData.status = getBusPermitStatus(validTo.trim());
    }
    if (routeFrom !== undefined) updateData.routeFrom = routeFrom.trim()
    if (routeTo !== undefined) updateData.routeTo = routeTo.trim()
    ...
    res.status(200).json({
      success: true,
      message: 'Bus permit updated successfully',
      data: updatedPermit
    })
  } catch (error) {
    console.error('Error updating Bus permit:', error)
...
exports.deletePermit = async (req, res) => {
  try {
    ...
    const deletedPermit = await BusPermit.findOneAndDelete({ _id: id, userId: req.user.id })
    ...
    res.status(200).json({
      success: true,
      message: 'Bus permit deleted successfully',
      data: deletedPermit
    })
  } catch (error) {
    console.error('Error deleting Bus permit:', error)
...
exports.getStatistics = async (req, res) => {
  try {
    ...
    const activePermits = await BusPermit.countDocuments({ status: 'active', userId: req.user.id })

    // For expiring soon and expired, exclude vehicles that also have active permits (renewed vehicles)
    const vehiclesWithActivePermits = await BusPermit.find({ status: 'active', userId: req.user.id })
      .distinct('vehicleNumber')

    const expiringPermits = await BusPermit.countDocuments({
      userId: req.user.id,
      status: 'expiring_soon',
      vehicleNumber: { $nin: vehiclesWithActivePermits }
    })

    const expiredPermits = await BusPermit.countDocuments({
      userId: req.user.id,
      status: 'expired',
      vehicleNumber: { $nin: vehiclesWithActivePermits }
    })

    const total = await BusPermit.countDocuments({ userId: req.user.id })
    ...
    const pendingPaymentResults = await BusPermit.aggregate(pendingPaymentPipeline)
    ...
    console.error('Error fetching Bus permit statistics:', error)
    logError(error, req)
    const userError = getUserFriendlyError(error)
    res.status(500).json({
      success: false,
...
// Mark Bus permit as paid
exports.markAsPaid = async (req, res) => {
  try {
    const permit = await BusPermit.findOne({ _id: req.params.id, userId: req.user.id })
    ...
    if (!permit.balance || permit.balance === 0) {
      return res.status(400).json({
        success: false,
        message: 'No pending payment for this Bus permit'
      })
    }
