...
Thank you!`
  ...
  const handleIssuePermit = async (formData) => {
    try {
      ...
      if(error.response.data){
        return toast.error(`${error.response.data.message || 'some error while adding temp permit'}`, {
        position: 'top-right',
        autoClose: 3000
      })
      }
      ...
      <div className='bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden'>
        ...
        <div className='px-6 py-5 bg-gradient-to-r from-indigo-50 via-purple-50 to-pink-50 border-b border-gray-200'>
          <div className='flex flex-col lg:flex-row gap-2 items-stretch lg:items-center'>
            ...
            {/* New Permit Button */}
            <AddButton onClick={() => setShowIssuePermitModal(true)} title='New Temporary Permit' />
          </div>
        </div>
