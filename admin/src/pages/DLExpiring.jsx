...
const DLExpiring = ({ setIsSidebarOpen }) => {
  ...
  // Fetch DL expiring soon applications from backend
  useEffect(() => {
    fetchApplications()
  ...
  const fetchApplications = async () => {
    try {
      ...
      const response = await drivingLicenseAPI.getDLExpiringSoon(params)

      console.log('DL Expiring API Response:', response)
      ...
      const transformedData = (response.data || []).map(app => {
        ...
        let formattedDLExpiryDate = '-'
        try {
          if (app.LicenseExpiryDate) {
            const d = new Date(app.LicenseExpiryDate)
            if (!isNaN(d.getTime())) {
              const day = String(d.getDate()).padStart(2, '0')
              const month = String(d.getMonth() + 1).padStart(2, '0')
              const year = d.getFullYear()
              formattedDLExpiryDate = `${day}-${month}-${year}`
            }
          }
        } catch (err) {
          console.error('DL Expiry date parsing error:', err)
        }

        // Format DL issue date
        let formattedDLIssueDate = '-'
        try {
          if (app.LicenseIssueDate) {
            const d = new Date(app.LicenseIssueDate)
            if (!isNaN(d.getTime())) {
              const day = String(d.getDate()).padStart(2, '0')
              const month = String(d.getMonth() + 1).padStart(2, '0')
              const year = d.getFullYear()
              formattedDLIssueDate = `${day}-${month}-${year}`
            }
          }
        } catch (err) {
          console.error('DL Issue date parsing error:', err)
        }
        ...
        try {
          if (app.LicenseExpiryDate) {
            const expiry = new Date(app.LicenseExpiryDate)
        ...
        return {
          ...
          date: formattedDate,
          licenseNumber: app.LicenseNumber || app.licenseNumber || '-',
          dlIssueDate: formattedDLIssueDate,
          dlExpiryDate: formattedDLExpiryDate,
          daysUntilExpiry,
      ...
      console.error('Error fetching DL expiring applications:', error)
      toast.error('Failed to fetch driving licenses. Please try again.', { autoClose: 700 })
      setApplications([])
      setTotalPages(0)
      setTotalItems(0)
    } finally {
        ...
        <div className='w-full px-3 md:px-4 lg:px-6 pt-20 lg:pt-20 pb-8'>
          ...
          <div className='mb-6'>
            <div className='flex items-center gap-3'>
              <button
                onClick={() => navigate('/driving-licence')}
              ...
              <div>
                <h1 className='text-2xl md:text-3xl font-black text-gray-800'>Driving Licenses Expiring Soon</h1>
                <p className='text-sm text-gray-600 mt-1'>Driving licenses expiring within the next 30 days</p>
              </div>
            </div>
          </div>
              ...
              <table className='w-full'>
                <thead className='bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600'>
                  <tr>
                    ...
                    <th className='px-4 py-4 text-left text-xs font-bold text-white uppercase tracking-wide'>DL Number</th>
                    <th className='px-4 py-4 text-left text-xs font-bold text-white uppercase tracking-wide'>DL Issue Date</th>
                    <th className='px-4 py-4 text-left text-xs font-bold text-white uppercase tracking-wide'>DL Expiry Date</th>
                    ...
                    <tr>
                      <td colSpan='10' className='px-4 py-8 text-center'>
                        <div className='text-gray-400'>
                          ...
                          <p className='text-sm font-semibold text-gray-600'>No driving licenses expiring soon</p>
                          <p className='text-xs text-gray-500 mt-1'>All driving licenses are valid for more than 30 days</p>
                        </div>
                      </td>
                    </tr>
                  )}
