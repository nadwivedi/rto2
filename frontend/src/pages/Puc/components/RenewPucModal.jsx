...
const RenewPucModal = ({ isOpen, onClose, onSubmit, oldPuc }) => {
  ...
  // Populate form when oldPuc record changes
  useEffect(() => {
    if (oldPuc) {
      ...
      setFormData({
        ...
        validFrom: todayStr,
        validTo: '',
        totalFee: oldPuc.totalFee?.toString() || '0',
        paid: '0',
        balance: oldPuc.totalFee?.toString() || '0'
      })

      if (oldPuc.vehicleNumber) {
        const validation = validateVehicleNumberRealtime(oldPuc.vehicleNumber)
          ...
          <div className='flex justify-between items-center'>
            <div>
              <h2 className='text-lg md:text-2xl font-bold'>
                Renew PUC Certificate
              </h2>
              <p className='text-green-100 text-xs md:text-sm mt-1'>
                Renew PUC certificate for same vehicle
              </p>
            </div>
            <button
              onClick={onClose}
