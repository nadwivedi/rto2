...
const AddPucModal = ({ isOpen, onClose, onSubmit, prefilledVehicleNumber = '', prefilledOwnerName = '', prefilledMobileNumber = '' }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${API_URL}/api/puc`, dataToSubmit, {
      ...
      console.error('Error adding PUC:', error)
      toast.error(error.response?.data?.message || 'Failed to add PUC record')
    } finally {
          ...
          <div className='flex justify-between items-center'>
            <div>
              <h2 className='text-lg md:text-2xl font-bold'>
                Add New PUC Certificate
              </h2>
              <p className='text-green-100 text-xs md:text-sm mt-1'>
                Add Pollution Under Control certificate record
              </p>
            </div>
            <button
              onClick={onClose}
