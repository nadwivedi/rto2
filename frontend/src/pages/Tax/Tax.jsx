  ...
  const handleWhatsAppClick = async (record) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `${API_URL}/api/tax/${record.id}/whatsapp-increment`,
        {},
        { withCredentials: true }
      );
      ...
      if (response.data.success) {
        ...
        setTaxRecords(prevRecords =>
          prevRecords.map(r =>
            r.id === record.id
              ? { ...r, whatsappMessageCount: response.data.data.whatsappMessageCount }
              : r
          )
        );
      }
    ...
    let message = `Hello ${record.ownerName || ''},\n\n`;
                      ...
                      render: (record) => {
                        ...
                        return (
                          <div className='flex items-center gap-1.5 pt-2 pb-1'>
                            <div className='flex items-center gap-0.5 bg-green-50 px-2 py-1 rounded-full border border-green-200'>
