...
const Gps = () => {
  ...
  const fetchStatistics = async () => {
    try {
      const response = await axios.get(`${API_URL}/api/gps/statistics`, { withCredentials: true });
  ...
  // Fetch GPS records from API
  const fetchGpsRecords = async (page = pagination.currentPage) => {
    setLoading(true);
    let url = `${API_URL}/api/gps`;
    const params = {
      page,
    ...
    if (statusFilter !== "all") {
      ...
      url = `${API_URL}/api/gps/${filterPath}`;
    }
    ...
    try {
      ...
      toast.error(
        "Failed to fetch GPS records. Please check if the backend server is running.",
        {
          position: "top-right",
          autoClose: 3000,
        }
  ...
  // Load GPS records and statistics on component mount and when filters change
  useEffect(() => {
    fetchGpsRecords(1); // Reset to page 1 when filters change
    fetchStatistics(); // Fetch fresh statistics
  }, [searchQuery, statusFilter]);

  // Page change handler
  const handlePageChange = (newPage) => {
    fetchGpsRecords(newPage);
  ...
  const handleEditGps = async (formData) => {
    ...
    try {
      ...
      const gpsId = selectedGps._id || selectedGps.id;

      const response = await axios.put(
        `${API_URL}/api/gps/id/${gpsId}`,
        {
      ...
      if (response.data.success) {
        toast.success("GPS certificate updated successfully!", {
          position: "top-right",
          autoClose: 3000,
        });
      ...
      const errorMessage = error.response?.data?.message || "Failed to update GPS certificate.";
  ...
  const handleDeleteGps = async (record) => {
    ...
    const confirmDelete = window.confirm(
      `Are you sure you want to delete this GPS certificate?\n\n` +
    ...
    try {
      const response = await axios.delete(
        `${API_URL}/api/gps/id/${record.id}`,
        { withCredentials: true }
      );

      if (response.data.success) {
        toast.success("GPS certificate deleted successfully!", {
          position: "top-right",
          autoClose: 3000,
        });
        ...
        throw new Error(
          response.data.message || "Failed to delete GPS certificate"
        );
      }
    } catch (error) {
      console.error("Error deleting GPS certificate:", error);
      toast.error(`Failed to delete GPS certificate: ${error.message}`, {
        position: "top-right",
        autoClose: 3000,
      });
    } finally {
  ...
  const handleMarkAsPaid = async (record) => {
    ...
    try {
      ...
      const response = await axios.patch(`${API_URL}/api/gps/id/${record.id}/mark-as-paid`, {}, { withCredentials: true });
      ...
      // Refresh the GPS records list and statistics
      await fetchGpsRecords();
      await fetchStatistics();
    } catch (error) {
  ...
  const handleWhatsAppClick = (record) => {
    ...
    // Create custom message for expiring GPS only
    let message = `Hello,\n\n`;
    ...
    if (record.status === 'expiring_soon' || record.status === 'expired') {
      ...
      message += `Your GPS certificate ${statusText} on ${record.validTo}.\n`;
      message += `Vehicle Number: ${record.vehicleNumber}\n`;
      message += `Please renew your GPS at the earliest.\n\n`;
    }
  ...
  return (
    <>
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden">
            <div className="px-6 py-5 bg-gradient-to-r from-indigo-50 via-purple-50 to-pink-50 border-b border-gray-200">
              ...
              <div className="mt-3 text-xs text-gray-600 font-semibold">
                Showing {gpsRecords.length} of {pagination.totalRecords}{" "}
                records
              </div>
            </div>
            ...
            <MobileCardView
              records={gpsRecords}
              emptyMessage={{
                title: 'No GPS records found',
                description: 'Click "Add New" to add your first record',
              }}
              loadingMessage='Loading GPS records...'
              cardConfig={{
                header: {
                  avatar: null,
                      ...
                      >
                        ...
                        <td className="px-1 2xl:px-2 py-3 2xl:py-4">
                          <div className="flex items-center justify-end gap-0.5 2xl:gap-0.5 pr-1">
                            ...
                            {/* Delete Button */}
                            <button
                              onClick={() => handleDeleteGps(record)}
      ...
      {/* Add GPS Modal - Lazy Loaded */}
      {isAddModalOpen && (
        <AddGpsModal
          isOpen={isAddModalOpen}
          onClose={() => setIsAddModalOpen(false)}
          onSubmit={handleAddGps}
        />
      )}

      {/* Edit GPS Modal - Lazy Loaded */}
      {isEditModalOpen && (
        <EditGpsModal
