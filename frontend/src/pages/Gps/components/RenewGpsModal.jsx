...
const RenewGpsModal = ({ isOpen, onClose, onSubmit, oldGps }) => {
  ...
  // Populate form when oldGps record changes
  useEffect(() => {
    if (oldGps) {
      ...
      setFormData({
        oldGpsId: oldGps._id || oldGps.id,
        vehicleNumber: oldGps.vehicleNumber || '',
        ownerName: oldGps.ownerName || '',
        mobileNumber: oldGps.mobileNumber || '',
        validFrom: todayStr,
        validTo: '',
        totalFee: oldGps.totalFee?.toString() || '0',
        paid: '0',
        balance: oldGps.totalFee?.toString() || '0'
      })

      if (oldGps.vehicleNumber) {
        const validation = validateVehicleNumberRealtime(oldGps.vehicleNumber)
