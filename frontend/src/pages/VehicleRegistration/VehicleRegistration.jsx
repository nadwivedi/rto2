  ...
  const fetchRegistrations = async (page = pagination.currentPage) => {
    try {
      ...
      if (response.data.success) {
        // Transform data to include WhatsApp tracking fields
        const transformedData = (response.data.data || []).map(reg => ({
          ...reg,
...
⚖️ Weight Information:
  ...
  const handleWhatsAppReminder = async (registration) => {
    if (!registration.mobileNumber) {
      toast.error('Mobile number not available for this registration', {
        position: 'top-right',
        autoClose: 3000
      });
      return;
    ...
    let phoneNumber = registration.mobileNumber.replace(/\D/g, '');
    ...
    // Create message
    const message = `Hello ${registration.ownerName || ''},\n\nThis is a reminder regarding your vehicle registration.\n\nVehicle Number: ${registration.vehicleNumber || registration.registrationNumber}\nChassis Number: ${registration.chassisNumber}\n\nThank you.`;

    // Increment WhatsApp message count in backend
    try {
      const response = await axios.patch(
        `${API_URL}/api/vehicle-registrations/${registration._id}/whatsapp-increment`,
        {},
        { withCredentials: true }
      );
                  ...
                  <div className='p-4 space-y-3'>
                    {filteredRegistrations.map((registration) => (
                      <div key={registration._id} className='bg-white rounded-lg shadow border border-gray-200 overflow-hidden hover:shadow-md transition-shadow'>
                        ...
                        <div className='bg-gray-50 p-3 flex items-start justify-between border-b border-gray-200'>
                          ...
                          <div className='flex items-center gap-1'>
                            {registration.mobileNumber && (
                              <button
                                onClick={() => handleWhatsAppReminder(registration)}
                        ...
                        {/* WhatsApp Tracking Footer */}
                        {registration.whatsappMessageCount > 0 && (
                          <div className='bg-green-50 border-t border-green-100 py-2.5 px-3'>
                              ...
                              <div className='flex items-center gap-1.5'>
                                <div className='flex items-center gap-0.5 bg-green-100 px-2.5 py-1 rounded-full border border-green-200'>
                                  ...
                                  <span className='text-xs font-semibold text-green-700'>
                                    {registration.whatsappMessageCount === 1 && '✓'}
                                    {registration.whatsappMessageCount === 2 && '✓✓'}
                                    {registration.whatsappMessageCount >= 3 && '✓✓✓'}
                                    {registration.whatsappMessageCount > 3 && ` (${registration.whatsappMessageCount})`}
                                  </span>
                                </div>
                                <span className='text-[10px] text-gray-600 font-medium'>
                                  {registration.whatsappMessageCount === 1 ? '1 reminder sent' : `${registration.whatsappMessageCount} reminders sent`}
                                </span>
                              </div>
                              {registration.lastWhatsappSentAt && (
                                <div className='flex items-center gap-1'>
                                  <svg className='w-3 h-3 text-gray-500' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
                                    <path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z' />
                                  </svg>
                                  <span className='text-[10px] text-gray-600 font-medium'>
                                    {formatWhatsAppDate(registration.lastWhatsappSentAt)}
                                  </span>
                                </div>
                              )}
                            </div>
